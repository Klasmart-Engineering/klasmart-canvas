...
describe('Flood fill Tool', () => {
  ...
  it(`should select a secondary color and change the color of the shape`, async () => {
    ...
    const floodFillArrow = getByTestId(
      container,
  ...
  it(`should flood fill a shape`, async () => {
    ...
    const defaultFillButton = getByTestId(
      container,
      'special-selector-flood_fill'
    );

    fireEvent.click(shapeButton);
    fireEvent.mouseDown(upperCanvas, { clientX: 60, clientY: 200 });
    fireEvent.mouseMove(upperCanvas, { clientX: 200, clientY: 100 });
    fireEvent.mouseUp(upperCanvas, { clientX: 200, clientY: 100 });
  ...
  it(`should flood fill a free hand writing`, async () => {
    ...
    await wait(() => {
      ...
      expect(persistentEvents[persistentEvents.length - 1].type).toBe(
        'colorChanged'
      );
      expect(
        JSON.parse(persistentEvents[persistentEvents.length - 1].param).fill
      ).toBe('#000000');
    });
  });
});
