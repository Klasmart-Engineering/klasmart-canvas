...
const useSynchronizedPointer = (
  ...
  useEffect(() => {
    ...
    const move = (e: CanvasEvent) => {
      ...
      let laser: ICanvasObject | undefined = canvas?.getObjects().filter((o: ICanvasObject) => o.id === id)[0];
    ...
    if (laserIsActive && (universalPermits || allowPointer)) {
      canvas?.renderAll();
  ...
  }, [laserIsActive, canvas, allowPointer, eventSerializer, laserColor, universalPermits, userId]);
  ...
  useEffect(() => {
     ...
     */
    const moved = (id: string, target: { top: number, left: number, fill?: string }) => {
      if (!shouldHandleRemoteEvent(id)) {
        return;
      }

      const pointer = canvas?.getObjects().filter((o: ICanvasObject) => o.id === id)[0];
      const laser: TypedShape  = pointer ? pointer: laserPointer();

      if (!pointer) {
        laser.set({ selectable: false, evented: false, id, fill: target.fill || '#000' });
        canvas?.add(laser);
        canvas?.renderAll();     
      }
  ...
  }, [canvas, eventController, showPointer, universalPermits, shouldHandleRemoteEvent]);
}
