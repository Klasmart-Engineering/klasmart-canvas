...
function attachEventSerializer(
  controller: IPainterController,
  serializer: PaintEventSerializer
) {
...
export const WhiteboardContextProvider: FunctionComponent<Props> = ({
  children,
  userId,
  whiteboardId,
}: Props): JSX.Element => {
  const [brushParameters, setBrushParameters] = useState<BrushParameters>(
    BrushParameters.default()
  );
  const [pointerPainter, setPointerPainter] = useState<
    PointerPainterController | undefined
  >(undefined);
  ...
  useEffect(() => {
    if (!userId || !whiteboardId || !eventSerializer) return;
    ...
    pointerPainter.on('shapeBegin', (id: string, params: BrushParameters) =>
      shapesRepository.createShape(id, params)
    );
