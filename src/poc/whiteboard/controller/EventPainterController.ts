...
export class EventPainterController extends EventEmitter implements IPainterController {
    ...
    private parseAndEmitEvent(event: PainterEvent) {
        ...
        switch (event.type) {
        case "shapeBegin": this.beginShape(parsedParam as OperationData, event.id); break;
        case "shapeEnd": this.endShape(event.id); break;
        case "painterClear": this.clear(event.id); break;
        case "painterLine": this.line(event.id, parsedParam as LineData); break;
        }
    }

    private beginShape(operationData: OperationData, id: string) {
        if (id === undefined) {
