...
function ToolbarButton(props) {
    var id = props.id, title = props.title, iconSrc = props.iconSrc, iconName = props.iconName, active = props.active, onClick = props.onClick;
    ...
    return (React.createElement("button", { key: id, title: title, style: toolbarButtonStyle, className: [
            'toolbar-button original',
            active ? 'selected-button' : '',
            !active ? 'unselected-button' : '',
        ].join(' '), onClick: function () { return onClick(id); } },
        React.createElement("img", { src: iconSrc, alt: iconName, width: "24px", height: "24px" })));
}
