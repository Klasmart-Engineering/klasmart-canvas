...
export var WhiteboardCanvas = function (_a) {
    var children = _a.children, instanceId = _a.instanceId, userId = _a.userId, initialStyle = _a.initialStyle, pointerEvents = _a.pointerEvents, pixelWidth = _a.pixelWidth, pixelHeight = _a.pixelHeight, display = _a.display, scaleMode = _a.scaleMode, centerHorizontally = _a.centerHorizontally, centerVertically = _a.centerVertically;
    ...
    var generatedBy = useState(uuidv4())[0];
    var _f = useFixedAspectScaling(wrapper === null || wrapper === void 0 ? void 0 : wrapper.parentElement, pixelWidth / pixelHeight, scaleMode || 'ScaleToFit', centerHorizontally || false, centerVertically || false), width = _f.width, height = _f.height, top = _f.top, left = _f.left;
    // Event serialization for synchronizing whiteboard state.
    var _g = useSharedEventSerializer(), _h = _g.state, eventSerializer = _h.eventSerializer, eventController = _h.eventController, requestAllEvents = _g.requestAllEvents;
    var undoRedoDispatch = UndoRedo(canvas, eventSerializer, generatedBy, userId).dispatch;
    var _j = useContext(WhiteboardContext), text = _j.text, brushIsActive = _j.brushIsActive, textIsActive = _j.textIsActive, shapeIsActive = _j.shapeIsActive, fontFamily = _j.fontFamily, updateFontFamily = _j.updateFontFamily, fontColor = _j.fontColor, updateFontColor = _j.updateFontColor, penColor = _j.penColor, eraseType = _j.eraseType, shape = _j.shape, shapeColor = _j.shapeColor, lineWidth = _j.lineWidth, updateCanvasActions = _j.updateCanvasActions, shapesAreSelectable = _j.shapesAreSelectable, shapesAreEvented = _j.shapesAreEvented, eventedObjects = _j.eventedObjects, floodFillIsActive = _j.floodFillIsActive, updatePenColor = _j.updatePenColor, updateLineWidth = _j.updateLineWidth, updateShape = _j.updateShape, updateShapeColor = _j.updateShapeColor, floodFill = _j.floodFill, laserIsActive = _j.laserIsActive, allowPointer = _j.allowPointer, universalPermits = _j.universalPermits, toolbarIsEnabled = _j.toolbarIsEnabled, setToolbarIsEnabled = _j.setToolbarIsEnabled;
    var _k = useCanvasActions(userId, instanceId, generatedBy, canvas, undoRedoDispatch, eventSerializer), actions = _k.actions, mouseDown = _k.mouseDown;
    /**
    ...
    var filterIncomingEvents = useCallback(function (_id, eventGeneratedBy) {
        if (!eventGeneratedBy)
            return false;
