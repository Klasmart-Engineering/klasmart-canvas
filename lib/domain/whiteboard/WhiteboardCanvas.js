...
export var WhiteboardCanvas = function (_a) {
    ...
    useEffect(function () {
        if (wrapper && lowerCanvas && upperCanvas) {
            var pointerEventsStyle = (pointerEvents && !clickThrough) ? 'auto' : 'none';
        ...
        if (canvas && fontFamily) {
            ...
            if (type_1 === 'textbox' && obj) {
                ...
                var mappedObjects = canvas === null || canvas === void 0 ? void 0 : canvas.getObjects().map(function (object) {
                    ...
                    return __assign(__assign({}, transformed), { top: top, left: left, scaleX: options.scaleX, scaleY: options.scaleY });
                });
                undoRedoDispatch({
                    type: SET_GROUP,
    ...
    useEffect(function () {
        var registerId = instanceId;
        var registerActions = actions;
        updateCanvasActions({ op: "add", id: registerId, value: registerActions });
        return function () {
            updateCanvasActions({ op: "remove", id: registerId, value: registerActions });
        };
    }, [actions, instanceId, updateCanvasActions]);
